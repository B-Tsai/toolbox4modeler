...
def openfoam_to_netcdf(
    ...
    """
    Reads 3D OpenFOAM data and converts it into a CF and UGRID compliant NetCDF file.
    
    This function reads native OpenFOAM unstructured polyhedral meshes and converts 
    them into the standard UGRID-1.0 topology structure, allowing tools like 
    QGIS, ParaView, and standard NetCDF readers to interpret the 3D grid correctly.
    
    Args:
        case_dir (str): Path to the OpenFOAM case.
        output_nc (str): Path for the output NetCDF file.
        ...
        include_boundaries (bool): If True, also extract and store boundary patch data 
                                   as a separate 2D mesh topology.
        
    Returns:
        xr.Dataset: The generated dataset.
    """
    case_path = Path(case_dir)
    foam_file = case_path / "openfoam.foam"
    
    # PyVista OpenFOAM reader requires a .foam file to exist
    ...
    # Read the first time step to extract mesh topology (assuming static mesh)
    ...
    try:
        # Fast vectorized approach (PyVista >= 0.40)
        ...
        # Create padded array and fill using a broadcasting mask
        ...
        # Slow fallback for older PyVista/VTK versions
        max_nodes = 0
        cell_nodes_list = []
    ...
    # =========================================================
    # BOUNDARY MESH TOPOLOGY (IF REQUESTED)
    # =========================================================
    ...
    if include_boundaries and "boundary" in mesh.keys():
        ...
        for patch_idx, patch_name in enumerate(boundary_blocks.keys()):
            ...
            for i in range(p_n_faces):
                ...
                # Offset the point IDs to create a single global node array for all boundaries
    ...
    # =========================================================
    ds = xr.Dataset()
    ds.coords['time'] = ('time', time_values, {'standard_name': 'time', 'units': 's'})
    
    # --- Internal Mesh Coordinates ---
    ds['node_x'] = ('node', nodes[:, 0], {'standard_name': 'projection_x_coordinate', 'units': 'm'})
    ds['node_y'] = ('node', nodes[:, 1], {'standard_name': 'projection_y_coordinate', 'units': 'm'})
    ds['node_z'] = ('node', nodes[:, 2], {'standard_name': 'projection_z_coordinate', 'units': 'm'})
    
    cell_centers = internal_mesh.cell_centers().points
    ds['volume_x'] = ('volume', cell_centers[:, 0], {'standard_name': 'projection_x_coordinate', 'units': 'm'})
    ds['volume_y'] = ('volume', cell_centers[:, 1], {'standard_name': 'projection_y_coordinate', 'units': 'm'})
    ds['volume_z'] = ('volume', cell_centers[:, 2], {'standard_name': 'projection_z_coordinate', 'units': 'm'})
    ...
    ds['volume_node_connectivity'] = (
        ('volume', 'max_node_per_volume'), 
        connectivity, 
        {'cf_role': 'volume_node_connectivity', 'start_index': 0, '_FillValue': -1}
    )
    
    # --- Boundary Mesh Coordinates and Topology ---
    if bnd_connectivity is not None:
        ds['bnd_node_x'] = ('bnd_node', bnd_nodes_array[:, 0], {'units': 'm'})
        ds['bnd_node_y'] = ('bnd_node', bnd_nodes_array[:, 1], {'units': 'm'})
        ds['bnd_node_z'] = ('bnd_node', bnd_nodes_array[:, 2], {'units': 'm'})
        
        # Boundary face centers extracted directly from OpenFOAM data
        ds['bnd_face_x'] = ('bnd_face', bnd_centers_array[:, 0], {'units': 'm'})
        ds['bnd_face_y'] = ('bnd_face', bnd_centers_array[:, 1], {'units': 'm'})
        ds['bnd_face_z'] = ('bnd_face', bnd_centers_array[:, 2], {'units': 'm'})
        ...
        ds['bnd_face_node_connectivity'] = (
            ('bnd_face', 'max_node_per_bnd_face'), 
            bnd_connectivity,
            {'cf_role': 'face_node_connectivity', 'start_index': 0, '_FillValue': -1}
        )
        
        # Save patch names for reference mapping
        ds['bnd_patch_name'] = ('bnd_patch', patch_names)
        ds['bnd_face_patch_id'] = ('bnd_face', bnd_face_patch_idx, {'long_name': 'Index mapping face to bnd_patch_name'})
    ...
    # Load CF standard variables mapping for OpenFOAM from JSON file
    import json
    ...
    # 1. Determine number of components for each variable from JSON metadata
    var_shapes = {}
    for var in variables:
        if var in KNOWN_METADATA and 'openfoam_type' in KNOWN_METADATA[var]:
            ...
            print(f"Warning: Variable '{var}' not found in openfoam_variables.json or missing 'openfoam_type'. Defaulting to scalar.")
            var_shapes[var] = 1
    ...
    for var, n_comp in var_shapes.items():
        ...
        # Add metadata if variable is known
            ...
            for var, n_comp in var_shapes.items():
                ...
                for patch_name in patch_names:
                    ...
                    if var in patch_mesh.cell_data:
                        ...
                        # Fallback if variable doesn't exist on this boundary
                        if n_comp == 1:
    ...
    # Add Global CF and UGRID Attributes
    ds.attrs['Conventions'] = 'CF-1.8 UGRID-1.0'
    ds.attrs['title'] = 'OpenFOAM 3D Volume and Boundary Data'
    ds.attrs['source'] = 'Generated by ofpost Python library'
    
    print(f"Saving CF/UGRID compliant dataset to {output_nc}...")
